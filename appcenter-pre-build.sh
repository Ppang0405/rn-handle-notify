...
if [ ! -z "$GOOGLE_SERVICES_PLIST" ]; then
  echo $GOOGLE_SERVICES_PLIST | base64 --decode > "$APPCENTER_SOURCE_DIRECTORY/ios/rnHandleNotify/GoogleService-Info.plist"
fi
...
if [ ! -z "$IOS_CODEPUSH_APPID" ]; then
  ...
  if [[ "$TRIGGER_IOS" != "" ]] || [[ $COMMIT_MESSAGE = *"[BUILD]"* ]]; then
    ...
    code-push release-react $IOS_CODEPUSH_APPID ios --outputDir ./build --plistFile ./ios/rnHandleNotify/Info.plist --description "$COMMIT_MESSAGE"
